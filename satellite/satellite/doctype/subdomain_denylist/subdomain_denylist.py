"""Subdomain Denylist — the brand/keyword block the guest routing API consults.

The Component-H complement to the frozen `RESERVED_SUBDOMAINS`: a brand or auth
keyword a tenant could otherwise grab under the valid regional wildcard cert
(phishing-as-a-service). `register`/`check_label` call `is_denylisted` inline; an
operator's new row is honored on the next call (no deploy, no migrate). Ported
verbatim from Atlas (spec/18 Component H) as a guest-plane concern.
"""
...
from satellite.routing.labels import normalize
...
def is_denylisted(label: str) -> bool:
	...
	A single indexed `exists` on the lowercased label — cheap enough to run inline on
...
def seed_denylist() -> int:
	...
	(a fresh test DB, the after_migrate seed) only fills the gaps. Returns the count
	inserted."""
	inserted = 0
